# Volta Protocol - Whitepaper V1

## Statut du document

Ce document fige la version de travail du mecanisme Volta avant developpement.
...
### 1.2 Ce que Volta ne doit pas etre
...
Pour eviter tout cycle infini, Volta introduit un verrou terminal.
...
Sans cooldown, la strategie optimale deviendrait un script de spam. Volta introduit donc un cout temporel propre a chaque wallet de session.
...
Volta utilise un wallet de session.
...
- sniper arme 0.1 SOL en escrow
- un adversaire tente deposit pour 0.1 SOL
- les 0.2 SOL entrent dans le pot
- pot brut devient 1.2 SOL
...
La compression du timer rend chaque seconde restante plus dense. Volta ne fabrique pas artificiellement l'urgence ; il code l'urgence dans la structure meme de la session.
...
La collusion n'est pas impossible, mais elle reste fragile. Plus la session est tendue, plus un accord hors chaine est facile a trahir au dernier moment. Volta ne promet pas d'abolir la collusion ; il cherche a la rendre instable et risquee.
...
Volta Protocol n'est pas un simple dernier-deposant-gagne. La version figee ici combine :
